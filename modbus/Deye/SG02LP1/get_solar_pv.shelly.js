...
function buildVirtualComponentsManifest() {
  ...
  for (i = 0; i < ENTITIES_SOLAR.length; i++) {
    key = 'p' + i;
    components.push({
      key: key,
      type: 'number',
      id: nextId,
      config: {
        name: ENTITIES_SOLAR[i].name,
        default_value: 0,
        unit: ENTITIES_SOLAR[i].units,
        persisted: false,
  ...
  return {
    ...
    groups: [
      { id: 200, name: 'Deye SG02LP1 Solar PV', components: groupMembers }
    ]
  };
}
...
function update() {
  ...
  for (i = 0; i < ENTITIES_SOLAR.length; i++) {
    value = ENTITIES_SOLAR[i].entity.getValue() * ENTITIES_SOLAR[i].scale;
    console.log(ENTITIES_SOLAR[i].name + ': ' + value + ' [' + ENTITIES_SOLAR[i].units + ']');
