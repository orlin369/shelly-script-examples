...
function buildVirtualComponentsManifest() {
  ...
  for (i = 0; i < ENTITIES_BATTERY.length; i++) {
    key = 'p' + i;
    components.push({
      key: key,
      type: 'number',
      id: nextId,
      config: {
        name: ENTITIES_BATTERY[i].name,
        default_value: 0,
        unit: ENTITIES_BATTERY[i].units,
        persisted: false,
  ...
  return {
    ...
    groups: [
      { id: 200, name: 'Deye SG02LP1 Battery', components: groupMembers }
    ]
  };
}
...
function update() {
  ...
  for (i = 0; i < ENTITIES_BATTERY.length; i++) {
    value = ENTITIES_BATTERY[i].entity.getValue() * ENTITIES_BATTERY[i].scale;
    console.log(ENTITIES_BATTERY[i].name + ': ' + value + ' [' + ENTITIES_BATTERY[i].units + ']');
