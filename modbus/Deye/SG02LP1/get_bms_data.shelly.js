...
function buildVirtualComponentsManifest() {
  ...
  for (i = 0; i < ENTITIES_BMS.length; i++) {
    key = 'p' + i;
    components.push({
      key: key,
      type: 'number',
      id: nextId,
      config: {
        name: ENTITIES_BMS[i].name,
        default_value: 0,
        unit: ENTITIES_BMS[i].units,
        persisted: false,
  ...
  return {
    ...
    groups: [
      { id: 200, name: 'Deye SG02LP1 BMS', components: groupMembers }
    ]
  };
}
...
function update() {
  ...
  for (i = 0; i < ENTITIES_BMS.length; i++) {
    value = ENTITIES_BMS[i].entity.getValue() * ENTITIES_BMS[i].scale;
    console.log(ENTITIES_BMS[i].name + ': ' + value + ' [' + ENTITIES_BMS[i].units + ']');
