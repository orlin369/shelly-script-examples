...
function buildVirtualComponentsManifest() {
  ...
  for (i = 0; i < ENTITIES_GRID.length; i++) {
    key = 'p' + i;
    components.push({
      key: key,
      type: 'number',
      id: nextId,
      config: {
        name: ENTITIES_GRID[i].name,
        default_value: 0,
        unit: ENTITIES_GRID[i].units,
        persisted: false,
  ...
  return {
    ...
    groups: [
      { id: 200, name: 'Deye SG02LP1 Grid', components: groupMembers }
    ]
  };
}
...
function update() {
  ...
  for (i = 0; i < ENTITIES_GRID.length; i++) {
    value = ENTITIES_GRID[i].entity.getValue() * ENTITIES_GRID[i].scale;
    console.log(ENTITIES_GRID[i].name + ': ' + value + ' [' + ENTITIES_GRID[i].units + ']');
