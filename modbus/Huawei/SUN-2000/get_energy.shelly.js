...
function buildVirtualComponentsManifest() {
  ...
  for (i = 0; i < SUN2000_ENERGY.length; i++) {
    key = 'p' + i;
    components.push({
      key: key,
      type: 'number',
      id: nextId,
      config: {
        name: SUN2000_ENERGY[i].name,
        default_value: 0,
        unit: SUN2000_ENERGY[i].units,
        persisted: false,
  ...
  return {
    ...
    groups: [
      { id: 200, name: 'Huawei SUN2000 Energy', components: groupMembers }
    ]
  };
}
