...
function buildVirtualComponentsManifest() {
  ...
  for (i = 0; i < SUN2000_GRID.length; i++) {
    key = 'p' + i;
    components.push({
      key: key,
      type: 'number',
      id: nextId,
      config: {
        name: SUN2000_GRID[i].name,
        default_value: 0,
        unit: SUN2000_GRID[i].units,
        persisted: false,
  ...
  return {
    ...
    groups: [
      { id: 200, name: 'Huawei SUN2000 Grid', components: groupMembers }
    ]
  };
}
