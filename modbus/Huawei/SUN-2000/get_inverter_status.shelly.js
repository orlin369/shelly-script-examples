...
function buildVirtualComponentsManifest() {
  ...
  for (i = 0; i < SUN2000_STATUS.length; i++) {
    key = 'p' + i;
    components.push({
      key: key,
      type: 'number',
      id: nextId,
      config: {
        name: SUN2000_STATUS[i].name,
        default_value: 0,
        unit: SUN2000_STATUS[i].units,
        persisted: false,
  ...
  return {
    ...
    groups: [
      { id: 200, name: 'Huawei SUN2000 Inverter Status', components: groupMembers }
    ]
  };
}
