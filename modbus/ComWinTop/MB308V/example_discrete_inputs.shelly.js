...
/*
    ...
    - Self-deploys a virtual component named DI0 (number, 0/1).
    - Add it to group named ComWinTop CWT-MB308V DI0 Example.
...
function rebuildModbusEndpoint() {
  ...
  ENTRY_DI = MODBUS_ENDPOINT.addEntity({ addr: 0, rtype: ModbusController.REGTYPE_DISCRETEINPUT, itype: "i16" });
}
...
var VIRTUAL_COMPONENTS = {
  ...
  groups: [
    { id: 200, name: 'ComWinTop CWT-MB308V DI0 Example', components: ['di0', 'slaveId'] }
  ]
};
...
function update(){
    ...
    let di_values = ENTRY_DI.getValue();

    // Update the virtual component with the new value
    if (vcHandles && vcHandles.di0) {
      vcHandles.di0.setValue(di_values ? 1 : 0);
    }

    // Print the voltage to the console.
    console.log("DI: " + di_values);
}
